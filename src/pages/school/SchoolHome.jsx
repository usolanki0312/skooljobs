...
const SchoolHome = () => {
  ...
  const scheduledInterviews = interviews.filter((iv) => iv.status !== "Cancelled");
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1fr_320px]">
        <div className="rounded-2xl border border-borderColor bg-white p-6 shadow-sm">
          ...
          <p className="mt-3 text-center text-xs text-slate-400">
            Posted: {jobs.filter((j) => j.status === "Active").length} · Shortlisted:{" "}
            {applicants.filter((a) => a.status === "Shortlisted").length} · Interviews: {scheduledInterviews.length}
          </p>
        </div>
            ...
            <div className="space-y-3">
              {scheduledInterviews.map((iv) => (
                <div key={iv.id} className="rounded-xl border border-borderColor bg-light p-3">
                  <p className="text-sm font-bold text-slate-800">{iv.candidateName}</p>
                  <p className="text-xs text-slate-500">{iv.jobTitle} · {iv.round}</p>
                  ...
                  <span
                    className={`mt-2 inline-block rounded-full px-2.5 py-0.5 text-xs font-bold ${
                      iv.status === "Confirmed" ? "bg-green-50 text-green-600" : "bg-amber-50 text-amber-600"
                    }`}
                  >
                    {iv.status}
                  </span>
